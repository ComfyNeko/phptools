  ...
  <a href="adminlogin.php" onclick="return confirm('Are you sure to logout?');">Logout</a>
	...
	<center> 
		...
		if(count($_POST)>0) {
               ...
               //changes made fromm here
              
                if($search >0)
                {
                $sql = "SELECT * FROM register WHERE EmployeeID='$search'";
                }
                else
                   {
                   $sql ="SELECT * FROM register WHERE certificate_id='$search'";
                    }
                

               if(mysqli_query($conn, $sql)){ 
			
                        //idk if you had a query here,if yes remove it.
                        $result = $conn->query($sql);
                        $conn->close(); 
                  //changed made till here      
		} else{ 
